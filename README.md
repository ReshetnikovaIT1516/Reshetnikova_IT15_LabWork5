...
2. Интерфейс Мяуканье: Mетод, который принимает набор объектов способных
   мяукать и вызывает мяуканье у каждого объекта. 
## Алгоритм решения
...
## Алгоритм решения
1. Чтение данных с проверкой формата:
    - Первая строка файла содержит количество учеников N
    - Проверка, что N является числом от 1 до 100
    - Проверка формата каждой строки: "Фамилия Имя"
    - Проверка соответствия фактического количества строк N

2. Подсчет фамилий и генерация логинов:
    - Использование HashMap для учета количества вхождений каждой фамилии
    - LinkedHashMap для сохранения порядка ввода (LinkedHashMap - это специальная версия HashMap, 
 которая сохраняет порядок добавления элементов)

3. Форматирование логинов:
    - Для первой встреченной фамилии: логин = фамилия
    - Для повторяющихся фамилий: логин = фамилия + номер повторения
...
* TreeSet - это коллекция Java, которая:
-  Автоматически сортирует элементы в естественном порядке
-  Хранит только уникальные элементы (удаляет дубликаты)
-  Использует красно-черное дерево для эффективного хранения
